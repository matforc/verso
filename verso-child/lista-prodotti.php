...
<div class="page-wrapper <?php echo esc_attr($page_padding); ?>">
    <div class="<?php echo esc_attr($page_container); ?> layout-<?php echo esc_attr($page_layout); ?>">
        ...
        <div class="post-content <?php if ( $page_layout !='fullwidth' ) { ?> col-md-8  <?php }else{?> col-md-12 <?php } ?>">
            ...
            /* Post List START */
            <?php
            //For each custom field called product_list_cat_ids_* (the * is for product specific name), I create a product list query based on it
            // La Query
            $meta = get_post_meta( get_the_ID() );
            foreach ($meta as $key => $value) {
                $keyString = (string)$key;
                if(strpos($keyString, 'product_list_cat_ids_') === 0){
                    $sections[$key] =  $value;
                }
            }
            ...
            *
                ...
                Array
            (
                [product_list_cat_ids_buste] => Array
                    (
                        [0] => [772,771,77]
                    )
