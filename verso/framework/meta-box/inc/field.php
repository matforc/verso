...
abstract class RWMB_Field
{
	...
	public static function begin_html( $meta, $field )
	{
		...
		if ( $field['name'] )
		{
			...
			$field_label = sprintf(
				'<div class="rwmb-label"><label for="%s"><strong>%s</strong></label>%s</div>',
				$field['id'],
				$field['name'],
				$desc
			);
	...
	{
		$text = apply_filters( 'rwmb_add_clone_button_text', esc_html__( '+ Add more', 'santos' ), $field );
