...
class RWMB_File_Field extends RWMB_Field
{
	...
	public static function admin_enqueue_scripts()
	{
		...
		if ( ! $wp_scripts->get_data( 'rwmb-file', 'data' ) )
		{
			wp_localize_script( 'rwmb-file', 'rwmbFile', array(
				'maxFileUploadsSingle' => esc_html__( 'You may only upload maximum %d file', 'santos' ),
				'maxFileUploadsPlural' => esc_html__( 'You may only upload maximum %d files', 'santos' ),
			) );
		}
	}
	...
	public static function wp_ajax_delete_file()
	{
		...
		if ( $success )
			wp_send_json_success();
		wp_send_json_error( esc_html__( 'Error: Cannot delete file', 'santos' ) );
	}
	...
	{
		$i18n_title = apply_filters( 'rwmb_file_upload_string', _x( 'Upload Files', 'file upload', 'santos' ), $field );
		$i18n_more  = apply_filters( 'rwmb_file_add_string', _x( '+ Add new file', 'file upload', 'santos' ), $field );
	...
	{
		$i18n_delete = apply_filters( 'rwmb_file_delete_string', _x( 'Delete', 'file upload', 'santos' ) );
		$i18n_edit   = apply_filters( 'rwmb_file_edit_string', _x( 'Edit', 'file upload', 'santos' ) );
