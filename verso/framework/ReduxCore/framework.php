<?php
    ...
    if ( ! class_exists( 'ReduxFrameworkInstances' ) ) {
        // Instance Container
        require_once get_template_directory() . '/framework/ReduxCore/inc/class.redux_instances.php';
        require_once get_template_directory() . '/framework/ReduxCore/inc/lib.redux_instances.php';
    }
    ...
    if ( ! class_exists( 'ReduxFramework' ) ) {
        ...
        // Redux CDN class
        require_once get_template_directory() . '/framework/ReduxCore/inc/class.redux_cdn.php';

        // Redux API class  :)
        require_once get_template_directory() . '/framework/ReduxCore/inc/class.redux_api.php';

        // General helper functions
        require_once get_template_directory() . '/framework/ReduxCore/inc/class.redux_helpers.php';

        // General functions
        require_once get_template_directory() . '/framework/ReduxCore/inc/class.redux_functions.php';
        require_once get_template_directory() . '/framework/ReduxCore/inc/class.p.php';

        require_once get_template_directory() . '/framework/ReduxCore/inc/class.thirdparty.fixes.php';

        require_once get_template_directory() . '/framework/ReduxCore/inc/class.redux_filesystem.php';

        require_once get_template_directory() . '/framework/ReduxCore/inc/class.redux_admin_notices.php';
        ...
        class ReduxFramework {
            ...
            public static function init() {
                $dir = Redux_Helpers::cleanFilePath( get_template_directory() . '/framework/ReduxCore' );
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                ...
                if ( empty ( $this->args['footer_credit'] ) ) {
                    $this->args['footer_credit'] = '<span id="footer-thankyou">' . sprintf( __( 'Options panel created using %1$s', 'santos' ), '<a href="' . esc_url( $this->framework_url ) . '" target="_blank">' . __( 'Redux Framework', 'santos' ) . '</a> v' . self::$_version ) . '</span>';
                }

                if ( empty ( $this->args['menu_title'] ) ) {
                    $this->args['menu_title'] = __( 'Options', 'santos' );
                }

                if ( empty ( $this->args['page_title'] ) ) {
                    $this->args['page_title'] = __( 'Options', 'santos' );
                }
            ...
            private function _internationalization() {
                 ...
                 * @param string     'santos'  text domain
                 */
					$locale = apply_filters( "redux/textdomain/{$this->args['opt_name']}", get_locale(), 'santos' );
				 ...
				 load_textdomain( 'santos', self::$_dir . 'languages/' . $locale . '.mo' );
            }
            ...
            private function add_submenu( $page_parent, $page_title, $menu_title, $page_permissions, $page_slug ) {
                ...
                if ( isset ( $test[ $page_parent ] ) ) {
				 ...
				 $this->page = add_theme_page( $page_parent, $page_title, $menu_title, $page_permissions, $page_slug, array(&$this, 'generate_panel') );	
                    ...
                    if ( true === $this->args['allow_sub_menu'] ) {
                        if ( ! isset ( $section['type'] ) || $section['type'] != 'divide' ) {
                            foreach ( $this->sections as $k => $section ) {
                                ...
                                // ONLY for non-wp.org themes OR plugins. Theme-Check alert shown if used and IS theme.
                               add_theme_page($this->args['page_slug'], $section['title'], $section['title'], $this->args['page_permissions'], $this->args['page_slug'] . '&tab=' . $k, '__return_null');	
                            }
            ...
            public function _enqueue() {
				require_once get_template_directory() . '/framework/ReduxCore/core/enqueue.php';
            ...
            public function _load_page() {
                ...
                if ( true == $this->show_hints ) {
                    ...
                    $event = __( 'moving the mouse over', 'santos' );
                    if ( 'click' == $this->args['hints']['tip_effect']['show']['event'] ) {
                        $event = __( 'clicking', 'santos' );
                    }

                    // Construct message
                    $msg = sprintf( __( 'Hints are tooltips that popup when %d the hint icon, offering addition information about the field in which they appear.  They can be %d d by using the link below.', 'santos' ), $event, strtolower( $s ) ) . '<br/><br/>' . $url;

                    // Construct hint tab
                    $tab = array(
                        'id'      => 'redux-hint-tab',
                        'title'   => __( 'Hints', 'santos' ),
                        'content' => '<p>' . $msg . '</p>'
                    );
            ...
            private function get_default_output_string( $field ) {
                ...
                if ( ! empty ( $default_output ) ) {
                    $default_output = __( 'Default', 'santos' ) . ": " . substr( $default_output, 0, - 2 );
                }
            ...
            private function _register_extensions() {
                $path    = get_template_directory() . '/framework/ReduxCore/inc/extensions/';
            ...
            public function ajax_save() {
                if ( ! wp_verify_nonce( $_REQUEST['nonce'], "redux_ajax_nonce" . $this->args['opt_name'] ) ) {
                    echo json_encode( array(
                        'status' => __( 'Invalid security credential.  Please reload the page and try again.', 'santos' ),
                        'action' => ''
                    ) );
                ...
                if ( ! self::current_user_can( $this->args['page_permissions'] ) ) {
                    echo json_encode( array(
                        'status' => __( 'Invalid user capability.  Please reload the page and try again.', 'santos' ),
                        'action' => ''
                    ) );
                ...
                if ( ! empty ( $_POST['data'] ) && ! empty ( $redux->args['opt_name'] ) ) {
                    ...
                    if ( ! empty ( $values ) ) {
                        ...
                        echo json_encode( array( 'status' => __( 'Your panel has no fields. Nothing to save.', 'santos' ) ) );
                    }
                }
                if ( isset ( $this->transients['run_compiler'] ) && $this->transients['run_compiler'] ) {
                ...
                if ( isset( $return_array ) ) {
                    if ( $return_array['status'] == "success" ) {
						require_once get_template_directory() . '/framework/ReduxCore/core/panel.php';
                            ...
                            if ( is_array( $arr ) && ! empty( $arr ) ) {
                                foreach ( $arr as $x => $y ) {
                                    if ( strpos( strtolower( $y ), 'redux' ) !== false ) {
                                        $msg = __( '<strong>Redux Framework Notice: </strong>There are references to the Redux Framework support site in your config\'s <code>admin_bar_links</code> argument.  This is sample data.  Please change or remove this data before shipping your product.', 'santos' );
                            ...
                            if ( is_array( $arr ) && ! empty( $arr ) ) {
                                foreach ( $arr as $x => $y ) {
                                    if (!$this->omit_share_icons) {
                                        if ( strpos( strtolower( $y ), 'redux' ) !== false ) {
                                            $msg = __( '<strong>Redux Framework Notice: </strong>There are references to the Redux Framework support site in your config\'s <code>share_icons</code> argument.  This is sample data.  Please change or remove this data before shipping your product.', 'santos' );
